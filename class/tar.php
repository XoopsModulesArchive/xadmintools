...
class Archive_Tar extends PEAR
{
     ...
     * @param null   $p_compress can be null, 'gz' or 'bz2'. This
    ...
    public function __construct($p_tarname, $p_compress = null)
    {
        ...
        if (null === $p_compress) {
            if (@file_exists($p_tarname)) {
                ...
                if ('gz' == mb_substr($p_tarname, -2)) {
                    $this->_compress = true;

                    $this->_compress_type = 'gz';
                } elseif (('bz2' == mb_substr($p_tarname, -3)) ||
                          ('bz' == mb_substr($p_tarname, -2))) {
                    $this->_compress = true;
            ...
            if ((true === $p_compress) || ('gz' == $p_compress)) {
                $this->_compress = true;

                $this->_compress_type = 'gz';
            } elseif ('bz2' == $p_compress) {
                $this->_compress = true;
        ...
        if ($this->_compress) { // assert zlib or bz2 extension support
            if ('gz' == $this->_compress_type) {
                $extname = 'zlib';
            } elseif ('bz2' == $this->_compress_type) {
                $extname = 'bz2';
            }
    ...
    public function _openWrite()
    {
        ...
        } elseif ('none' == $this->_compress_type) {
    ...
    {
        if ('http://' == mb_strtolower(mb_substr($this->_tarname, 0, 7))) {
        ...
        if ($this->_file) {
            if (null === $p_len) {
                if ('gz' == $this->_compress_type) {
                    ...
                    @fwrite($this->_file, $p_binary_data);
                } else {
                    $this->_error('Unknown or missing compression type (' . $this->_compress_type . ')');
                }
            } else {
                if ('gz' == $this->_compress_type) {
                    ...
                    @fwrite($this->_file, $p_binary_data, $p_len);
                } else {
                    $this->_error('Unknown or missing compression type (' . $this->_compress_type . ')');
                }
            }
    ...
    public function _addFile($p_filename, &$p_header, $p_add_dir, $p_remove_dir)
    {
        ...
        if (0 == strcmp($p_filename, $p_remove_dir)) {
            return true;
        }
    ...
    public function _addString($p_filename, $p_string)
    {
        ...
        if (!$this->_writeHeaderBlock($p_filename, mb_strlen($p_string), 0, 0, '', 0, 0)) {
            return false;
        }
    ...
    public function _extractInString($p_filename)
    {
        ...
        while (0 != mb_strlen($v_binary_data = $this->_readBlock())) {
            if (!$this->_readHeader($v_binary_data, $v_header)) {
                return null;
            }
            ...
            if ($v_header['filename'] == $p_filename) {
                ...
                if (($v_header['size'] % 512) != 0) {
                    ...
                    $v_result_str .= mb_substr($v_content, 0, ($v_header['size'] % 512));
                }
    ...
    public function _extractList($p_path, &$p_list_detail, $p_mode, $p_file_list, $p_remove_path)
    {
        ...
        if ('' == $p_path || ('/' != mb_substr($p_path, 0, 1) && '../' != mb_substr($p_path, 0, 3) && !mb_strpos($p_path, ':'))) {
            $p_path = './' . $p_path;
        }
        ...
        while (0 != mb_strlen($v_binary_data = $this->_readBlock())) {
            ...
            && (mb_substr($v_header['filename'], 0, $p_remove_path_size) == $p_remove_path)) {
                ...
                if (file_exists($v_header['filename'])) {
                    ...
                    if ((is_file($v_header['filename'])) && ('5' == $v_header['typeflag'])) {
                ...
                elseif (1 != ($v_result = $this->_dirCheck(('5' == $v_header['typeflag'] ? $v_header['filename'] : dirname($v_header['filename']))))) {
    ...
    public function _openAppend()
    {
        ...
        if ($this->_compress) {
            ...
            if ('gz' == $this->_compress_type) {
                ...
                while (mb_strlen($v_buffered_lines[] = @bzread($v_temp_tar, 512)) > 0) {
