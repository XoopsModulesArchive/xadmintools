...
#####################################################################################  MODULE LIST ######################################################################
  require XOOPS_ROOT_PATH . '/modules/xadmintools/language/module_admin.php';
    ...
    $moduleHandler = xoops_getHandler('themes');
	...
	';
    $modules_dir = XOOPS_ROOT_PATH . '/themes';
    ...
    while ($file = readdir($handle)) {
        ...
        if ('' != $file && 'cvs' != mb_strtolower($file) && !preg_match('/^[.]{1,2}$/', $file) && is_dir($modules_dir . '/' . $file)) {
            if (!in_array($file, $listed_mods, true)) {
				...
				<td align="center" valign="bottom"><img src="' . XOOPS_URL . '/themes/' . $module->getInfo('dirname') . '/' . $module->getInfo('image') . '" alt="' . htmlspecialchars($module->getInfo('name'), ENT_QUOTES | ENT_HTML5) . '" border="0"></td>
